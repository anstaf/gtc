#pragma once
#include <gridtools/next/usid_cuda_helpers.hpp>
...
struct kernel_0 {
  GT_FUNCTION auto operator()() const {
    return [](auto &&ptr, auto &&strides, auto &&neighbors) {
      auto zavg = 0.5 * sum_neighbors<double, e2v_tag, 2, false>(
                            [](auto &&, auto &&n) { return field<pp_tag>(n); },
                            ptr, strides, neighbors);
...
struct kernel_1 {
  GT_FUNCTION auto operator()() const {
    return [](auto &&ptr, auto &&strides, auto &&neighbors) {
      field<pnabla_MXX_tag>(ptr) = sum_neighbors<double, v2e_tag, 7>(
          [](auto &&p, auto &&n) {
            return field<zavgS_MXX_tag>(n) * field<sign_tag>(p);
          },
          ptr, strides, neighbors);
      field<pnabla_MYY_tag>(ptr) = sum_neighbors<double, v2e_tag, 7>(
          [](auto &&p, auto &&n) {
            return field<zavgS_MYY_tag>(n) * field<sign_tag>(p);
          },
          ptr, strides, neighbors);
      field<pnabla_MXX_tag>(ptr) =
          field<pnabla_MXX_tag>(ptr) / field<vol_tag>(ptr);
      field<pnabla_MYY_tag>(ptr) =
          field<pnabla_MYY_tag>(ptr) / field<vol_tag>(ptr);
    };
  }
};
