...
namespace {
    ...
    constexpr auto storage_builder = gridtools::storage::builder<storage_traits_t>;
    ...
    const double rpi = 2 * std::asin(1);
    const double radius = 6371.22e+03;
    const double deg2rad = 2 * rpi / 360;
    constexpr int MXX = 0;
    constexpr int MYY = 1;
    constexpr auto edges_per_node = 7_c;

    auto make_vol(atlas::mesh::Nodes const &nodes) {
        ...
        return storage_builder        //
            .type<double>()           //
            .dimensions(nodes.size()) //
            .initializer(init)        //
            .build();                 //
    }

    auto make_sign(atlas::Mesh const &mesh) {
    ...
    auto make_S_MXX(atlas::mesh::Edges const &edges) {
    ...
    auto make_S_MYY(atlas::mesh::Edges const &edges) {
        auto dual_normals = atlas::array::make_view<double, 2>(edges.field("dual_normals"));
        return storage_builder                                                           //
            .type<double>()                                                              //
            .dimensions(edges.size())                                                    //
    ...
    auto make_pp(atlas::mesh::Nodes const &nodes) {
    ...
    auto min_max(T const &field) {
        ...
        return std::make_tuple(min, max, avg / (lengths[0] * lengths[1]));
    }
    ...
    TEST(FVM, nabla) {
        ...
        auto [x_min, x_max, x_avg] = min_max(pnabla_MXX);
        auto [y_min, y_max, y_avg] = min_max(pnabla_MYY);
