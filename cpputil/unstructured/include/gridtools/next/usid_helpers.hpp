...
namespace gridtools::next {
    ...
    GT_FUNCTION T fold_neighbors(F f, Init init, G g, Ptr &&ptr, Strides &&strides, Neighbors &&neighbors) {
        ...
        sid::make_loop<Tag>(integral_constant<int_t, N>())([&](auto const &ptr, auto &&) {
    ...
    template <class T, class Tag, int_t N, bool HasSkip = true, class F, class... Args>
    GT_FUNCTION T sum_neighbors(F f, Args &&...args) {
        return fold_neighbors<T, Tag, N, HasSkip>([](auto x, auto y) { return x + y; },
    ...
    GT_FUNCTION T min_neighbors(F f, Args &&...args) {
        return fold_neighbors<T, Tag, N, HasSkip>([](auto x, auto y) { return x < y ? x : y; },
