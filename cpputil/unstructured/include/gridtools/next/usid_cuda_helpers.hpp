...
#include "usid_helpers.hpp"

namespace gridtools::next::cuda {
    ...
    __global__ void kernel(int h_size, Ptr ptr_holder, Strides strides, Neighbors... neighbors) {
        auto h = blockIdx.x * blockDim.x + threadIdx.x;
        if (h >= h_size)
            return;
        Kernel()()(sid::shifted(ptr_holder(), device::at_key<dim::h>(strides), h),
            strides,
    ...
    void call_kernel(Size size, Sid &&fields, Sids &&...neighbor_fields) {
        ...
        kernel<Kernel><<<blocks, threads_per_block>>>(size,
