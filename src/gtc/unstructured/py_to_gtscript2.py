...
class _Visitor(ast.NodeVisitor):
    def _visit_list(self, src):
        return [self.visit(e) for e in src]

    def visit_Tuple(self, src: ast.Tuple):
        return self._visit_list(src.elts)
    ...
    def visit_With(self, src: ast.With):
        return gtscript_ast2.With(
            items=self._visit_list(src.items), body=self._visit_list(src.body)
        )

    def visit_Call(self, src: ast.Call):
        return gtscript_ast2.Call(func=self.visit(src.func), args=self._visit_list(src.args))
    ...
    def visit_GeneratorExp(self, src: ast.GeneratorExp):
        return gtscript_ast2.GeneratorExp(
            elt=self.visit(src.elt), generators=self._visit_list(src.generators)
        )
    ...
    def visit_Assign(self, src: ast.Assign):
        return gtscript_ast2.Assign(
            targets=self._visit_list(src.targets), value=self.visit(src.value)
        )

    def visit_FunctionDef(self, src: ast.FunctionDef):
        return gtscript_ast2.Function(name=src.name, body=self._visit_list(src.body))
...
class _FlattenWiths(eve.NodeTranslator):
    def _process_statements(self, src: List[gtscript_ast2.Stmt]):
        return (
            _flatten(
            ...
            else [self.visit(s) for s in src]
        )
    ...
    def visit_With(self, src: gtscript_ast2.With, items: List[gtscript_ast2.WithItem]):
